    ...
    override suspend fun measureDownloadSpeed(): Flow<Double> = flow {
        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                Log.d(
                    TAG,
                    "Response received - Success: ${response.isSuccessful}, Code: ${response.code}"
                )

                if (!response.isSuccessful || isCancelled) {
                    Log.e(
                        TAG,
                        "Download failed or cancelled - Success: ${response.isSuccessful}, Cancelled: $isCancelled"
                    )
                    emit(0.0)
                    return@flow
                }
                ...
                while (!isCancelled) {
                    ...
                    if (currentTime - lastEmitTime >= 200) {
                        ...
                        if (elapsedSeconds > 0) {
                            ...
                            Log.d(
                                TAG,
                                "Download progress - Bytes: $totalBytesRead, Speed: $speedMbps Mbps"
                            )
                            emit(speedMbps)
                ...
                if (totalElapsedSeconds > 0 && !isCancelled) {
                    ...
                    Log.d(
                        TAG,
                        "Download complete - Total bytes: $totalBytesRead, Time: $totalElapsedSeconds s, Final speed: $finalSpeedMbps Mbps"
                    )
                    emit(finalSpeedMbps)
                }
            }
    ...
    override suspend fun measureUploadSpeed(): Flow<Double> = flow {
        ...
        try {
            ...
            client.newCall(request).execute().use { response ->
                Log.d(
                    TAG,
                    "Upload response - Success: ${response.isSuccessful}, Code: ${response.code}"
                )

                if (!response.isSuccessful || isCancelled) {
                    Log.e(
                        TAG,
                        "Upload failed or cancelled - Success: ${response.isSuccessful}, Cancelled: $isCancelled"
                    )
                    emit(0.0)
                    return@flow
                }
                ...
                if (totalElapsedSeconds > 0 && !isCancelled) {
                    ...
                    Log.d(
                        TAG,
                        "Upload complete - Size: $uploadSize bytes, Time: $totalElapsedSeconds s, Speed: $speedMbps Mbps"
                    )
                    emit(speedMbps)
                }
            }
